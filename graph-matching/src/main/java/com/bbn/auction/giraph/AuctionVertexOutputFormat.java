...
public class AuctionVertexOutputFormat extends TextVertexOutputFormat<LongWritable, AuctionVertexValue, Writable> {
  ...
  protected class AuctionValueVertexWriter extends TextVertexWriterToEachLine {
    ...
    protected Text convertVertexToLine(Vertex<LongWritable, AuctionVertexValue, Writable, ?> vertex)
      throws IOException {
		Text line = new Text(vertex.getId().toString() + delimiter + vertex.getValue().toString());
		System.out.println(vertex.getId().toString() + delimiter + vertex.getValue().toString());
		return line;
    }
  }
}